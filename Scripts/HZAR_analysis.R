...
##              (8) Aggregate data for analysis and build plot
...
freq.loadavgmodel("fixed", "none", "modelI")
...
freq$avg$runs$init$modelI <-
  hzar.doFit(freq$avg$fitRs$init$modelI) # acc. rate=0.18855
# Plot the trace:
...
## runif(15,0,1) pMax for modelII, modelIII
...
# hzar.plot.cline(freq$avg$analysis$oDG$data.groups$modelI)
hzar.plot.fzCline(freq$avg$analysis$oDG$data.groups$modelI) # save this plot 6x11

# Overlay the above plot with the one generated from the XXX.R script so that samples are present
# with consistent coloration
